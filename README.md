...
## 7.1. Деплой бота на Koyeb (альтернатива Railway, бесплатный тариф)

Если вы хотите уйти с Railway, близкий по возможностям вариант — Koyeb. Он позволяет запускать фоновые **Worker**‑сервисы из Git‑репозитория или Docker‑образа; Supabase остаётся как есть, миграция БД не требуется.
...
3. **Задайте переменные окружения**
   - В разделе **Environment variables** добавьте:
     ...
     - `SUPABASE_KEY` — service_role ключ для бота (как и для Railway).
...
После успешного деплоя бот будет работать аналогично Railway‑инстансу: как фоновый long polling‑процесс, который обрабатывает обновления и планировщик закрытия туров.
