...
Переменные окружения — те же, что у бота (`SUPABASE_URL`, `SUPABASE_KEY`). Опционально можно задать `API_KEY` и тогда запросы к API должны содержать заголовок `X-API-Key`. Для запросов из браузера (фронт, Mini App) задайте **`CORS_ORIGINS`** — через запятую список доверенных origin'ов (например `http://localhost:5173` для разработки и URL фронта для продакшена). Для защиты от IDOR чувствительные операции требуют заголовок **`X-Player-Id`** (идентификатор игрока от имени которого выполняется запрос): обновление имени игрока, запрос pending-матчей, подтверждение/отклонение матча, уведомление соперника и т.д. См. [docs/SECURITY_OWASP_ANALYSIS.md](docs/SECURITY_OWASP_ANALYSIS.md).
