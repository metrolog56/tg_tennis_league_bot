...
function _requireApiUrl(feature = 'операция') {
  const apiUrl = (import.meta.env.VITE_API_URL || '').trim()
  if (!apiUrl) {
    throw new Error(`Недоступно: ${feature}. Обновите приложение или обратитесь к администратору (API не настроен).`)
  }
  return apiUrl
}
...
export async function updatePlayerName(playerId, newName) {
  ...
  _requireApiUrl('изменение имени')
}
...
export async function submitMatchForConfirmation(divisionId, player1Id, player2Id, sets1, sets2, submittedBy) {
  ...
  _requireApiUrl('внесение результата матча')
}
...
export async function confirmMatchResult(matchId, confirmedByPlayerId) {
  ...
  _requireApiUrl('подтверждение результата')
}
...
export async function rejectMatchResult(matchId, rejectedByPlayerId) {
  ...
  _requireApiUrl('отклонение результата')
}

export async function submitMatchResult(divisionId, player1Id, player2Id, sets1, sets2, submittedBy = null) {
  _requireApiUrl('внесение сыгранного матча')
  throw new Error('Недоступно: внесение сыгранного матча напрямую из клиента отключено. Используйте сценарий с подтверждением результата.')
}
