...
/** Single match by id with player names (for confirm screen). */
export async function getMatchById(matchId) {
  ...
  const [p1, p2] = await Promise.all([
    supabase.from('players').select('id, name, telegram_id').eq('id', match.player1_id).maybeSingle(),
    supabase.from('players').select('id, name, telegram_id').eq('id', match.player2_id).maybeSingle(),
  ])
  return {
    ...match,
    player1: p1.data || null,
    player2: p2.data || null,
  }
}

/** Matches where status is pending_confirm and the given player is the opponent (must confirm). */
export async function getPendingConfirmationForPlayer(playerId) {
  const { data, error } = await supabase
    .from('matches')
  ...
  if (error) throw error
  const rows = data || []
  return rows.filter((m) => m.submitted_by !== playerId)
}
...
async function applyMatchResultAsPlayed(match) {
  const divisionId = match.division_id
  const player1Id = match.player1_id
  const player2Id = match.player2_id
  const sets1 = Number(match.sets_player1) || 0
  const sets2 = Number(match.sets_player2) || 0
  ...
  const winnerId = sets1 > sets2 ? player1Id : player2Id
  const loserId = sets1 > sets2 ? player2Id : player1Id
  const winnerSets = sets1 > sets2 ? sets1 : sets2
  const loserSets = sets1 > sets2 ? sets2 : sets1
  const winnerRatingBefore = sets1 > sets2 ? r1Val : r2Val
  const loserRatingBefore = sets1 > sets2 ? r2Val : r1Val
...
export async function confirmMatchResult(matchId, confirmedByPlayerId) {
  ...
  const submitterId = row.submitted_by
  if (confirmedByPlayerId !== row.player1_id && confirmedByPlayerId !== row.player2_id) throw new Error('Вы не участник этого матча.')
  if (confirmedByPlayerId === submitterId) throw new Error('Подтверждать должен соперник.')
  await applyMatchResultAsPlayed(row)
}
...
export async function rejectMatchResult(matchId, rejectedByPlayerId) {
  ...
  if (fetchError) throw fetchError
  if (!row || row.status !== 'pending_confirm') throw new Error('Матч не найден или уже обработан.')
  if (rejectedByPlayerId !== row.player1_id && rejectedByPlayerId !== row.player2_id) throw new Error('Вы не участник этого матча.')
  if (rejectedByPlayerId === row.submitted_by) throw new Error('Отклонить может только соперник.')
